...
const GenerateWeeklyContentOutputSchema = z.object({
  ...
  games: z.array(GameSchema).describe('An array of 3-5 interactive games based on the sermon. Include a mix of types like Quiz, Word Search, Fill in the Blank, Matching, Word Guess, Wordle, or Jeopardy. For Quizzes, provide 3-4 questions with 4 multiple-choice options each. For Matching games, provide 4-6 pairs of terms and definitions. For Fill in the Blank, provide four key sentences with an important word missing. For Word Guess, provide a single key word and a hint for it. For Wordle, provide a single, relevant 5-letter word from the sermon. For Jeopardy, create 2-3 categories with 3 questions each, with point values of 100, 200, and 300.'),
});
