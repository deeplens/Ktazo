...
const TwoTruthsAndALieItemSchema = z.object({
    statements: z.array(z.string()).length(3).describe('An array of three statements. Two are true, one is a lie.'),
    lieIndex: z.number().min(0).max(2).describe('The index (0, 1, or 2) of the lie in the statements array.'),
});

const GameSchema = z.object({
    type: z.enum(['Quiz', 'Word Search', 'Fill in the Blank', 'Matching', 'Word Guess', 'Wordle', 'Jeopardy', 'Verse Scramble', 'True/False', 'Word Cloud Hunt', 'Reflection Roulette', 'Two Truths and a Lie']),
    title: z.string(),
...
const GenerateWeeklyContentOutputSchema = z.object({
  ...
  games: z.array(GameSchema).length(12).describe("An array of exactly 12 interactive games based on the sermon. One game MUST be a 'Jeopardy' game. One game MUST be a 'Verse Scramble' game. One game MUST be a 'True/False' game with exactly 20 questions. One game MUST be a 'Word Cloud Hunt' with 15-20 key words. One game MUST be a 'Reflection Roulette' game; its data field should be null. One game MUST be 'Two Truths and a Lie'; generate 3-5 rounds for it. Include a mix of other types like Quiz, Word Search, Fill in the Blank, Matching, Word Guess, or Wordle. For Quizzes, provide 3-4 questions with 4 multiple-choice options each. For Matching games, provide 4-6 pairs of terms and definitions. For Fill in the Blank, provide four key sentences with an important word missing. For Word Guess, provide four key words from the sermon, each with a hint. For Wordle, provide a single, relevant 5-letter word from the sermon. For the required Jeopardy game, create 2-3 categories with 3 questions each, with point values of 200, 400, and 600."),
...
const generateWeeklyContentPrompt = ai.definePrompt({
  ...
  - An array of exactly 12 interactive games based on the sermon's content. 
    ...
    - One game must be 'Two Truths and a Lie'. Generate 3-5 rounds for it. Each round should have three statements: two that are true to the sermon, and one that is a plausible but incorrect 'lie'. Indicate the index of the lie.
    - Fill the remaining 6 slots with a mix of other game types like 'Quiz', 'Word Search', 'Fill in the Blank', 'Matching', 'Word Guess', or 'Wordle'. 
